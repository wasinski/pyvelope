...
from pyvelope import Consumer, Envelope, MessageBus
...
class ConsumerOfSecondCommand(Consumer[MySecondCommand]):
    def consume(self, message: Envelope[MySecondCommand]) -> None:  # noqa: PLR6301
...
assert sqs_client.send_message.call_count == 2  # noqa: PLR2004

message_bus.send(MyCommand("Hello, you address!"), SqsQueueUrl(EXAMPLE_SQS_QUEUE_URL))
assert sqs_client.send_message.call_count == 3  # noqa: PLR2004

message_bus.send(MyCommand("Hello, you bound address"), ConsumerOfSecondCommand)
assert sqs_client.send_message.call_count == 4  # noqa: PLR2004

# fake an incoming message and send a reply,
# faking is needed to have it working without a "real" message bus
...
assert sqs_client.send_message.call_count == 5  # noqa: PLR2004
